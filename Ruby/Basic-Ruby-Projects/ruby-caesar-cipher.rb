def caesar_cipher(input, shift)
  shift = shift % 26 #make sure the shift is in alphabet range
  input.each_char do |char|
    ...
    if ('a'..'z').include?(char) 
      if (char.ord + shift) > 122 #check if outside ASCII range
        char = ((char.ord + shift) - 26).chr  # shift will always be in alphabet range now, can just subtract 26 to wrap around
